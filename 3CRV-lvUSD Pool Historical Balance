...
select a.day, a.token_symbol, running_balance,  prct
from cealv_tvl a
...
, temp as (
select a.day, token_symbol, prct, running_balance, 
...
, lvusd_crv_ratio as (
select a.day, token_symbol, underlying_running_balance_usd as running_balance, '3CRV' as pair
from temp a
...
--running_balance,
'Total' pair,
sum(running_balance) over (partition by day order by day) as tvl,
--sum(running_balance) over (order by day) as pool_tvl,
...
select * from pool_tvl
where day >= now() - interval '{{Time Period}}'
order by day desc, pair desc
)

select distinct day, pair, tvl, ratio_pair from summarize_result
order by day desc, pair desc
